/**
 * SVG Icons as inline data-URL strings.
 *
 * In the TASK the user is told: use <img src="Search.svg" /> etc.
 * In reality (since we can't serve real files from an exercise),
 * we expose these as data URIs that the preview iframe can use via
 * window.__SVG__ injected into the iframe head.
 *
 * The user writes:  <img src="Search.svg" width="24" height="24" />
 * The preview wrapper replaces "IconName.svg" → the real data URI.
 */
...
/** Raw SVG strings — stroke="currentColor" so color can be overridden */
...
/** Returns a colored SVG data URI (replaces currentColor with the given hex) */
...
/** Returns the raw SVG string with currentColor replaced */
export function getSvgRaw(name: SvgIconName, color: string): string {
  return RAW[name].replace(/currentColor/g, color);
}

/**
 * Builds a JS snippet that injects all icon data-URIs as a global map
 * into the preview iframe, so the user can write:
 *   <img src="Search.svg" />
 * and it resolves correctly.
 *
 * We intercept via a MutationObserver that swaps src="Name.svg" → data URI.
 */
